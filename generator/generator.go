...
func generate(inFiles []string, outFile, packageComment string) error {
	output, err := os.OpenFile(outFile, os.O_CREATE|os.O_TRUNC|os.O_RDWR, 0755)
	if err != nil {
		panic(err)
	}
	...
	Run(&Config{
		OutputFile:      outFile,
		PackageComments: packageComment,
		Package:         "deepspeech",
		Prefix:          "",
		Ignore:          "",
		Include:         "",
...
type Config struct {
	...
	OutputFile string
	// Package comments for the generated file.
...
func Run(conf *Config, out io.Writer) error {
	...
	escFiles := make([]*_embeddedFile, 0, 10)
	...
	tmpl.Execute(buf, templateParams{
		...
		PackageComments: conf.PackageComments,
...
func (f *_embeddedFile) fillCompressed(gzipLevel int) error {
	...
	f.Compressed = res.String()
	return nil

}

const (
	fileTemplate = `{{.PackageComments}}// Code generated by "esc{{with .Invocation}} {{.}}{{end}}"; DO NOT EDIT.
package {{.PackageName}}
import (
	"bytes"
	"compress/gzip"
...
func openEmbeddedFile(name string) (*_embeddedFile, error) {
	f, present := _embeddedFiles[path.Clean(name)]
	if !present {
...
var _embeddedFiles = map[string]*_embeddedFile{
{{ range .Files }}
	"{{ .Name }}": {
		name:    "{{ .BaseName }}",
		local:   "{{ .Local }}",
